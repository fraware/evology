...
def learning_runs(repetitions, time, agents):
    ...
    while i < repetitions:
        ...
        try: 
            ...
            np.random.seed(seed)
            df,pop = main('scholl', solver, wealth_coordinates, time, agents, PROBA_SELECTION, MUTATION_RATE, ReinvestmentRate = 0, tqdm_display=True, reset_wealth = False)
...
def learning_runs_reset(repetitions, time, agents):
    ...
    while i < repetitions:
        ...
        print(wealth_coordinates)
        try: 
            df,pop = main('scholl', solver, wealth_coordinates, agents, time, PROBA_SELECTION, MUTATION_RATE, ReinvestmentRate = 0, tqdm_display=True, reset_wealth = True)
...
def nolearning_runs(repetitions, time, agents):
    ...
    while i < repetitions:
        try: 
            ...
            wealth_coordinates = np.random.dirichlet(np.ones(3),size=1)[0].tolist()
            print(wealth_coordinates)
            df,pop = main('scholl', solver, wealth_coordinates, agents, time, 0, 0, ReinvestmentRate = 0, tqdm_display=True, reset_wealth = False)
...
def nolearning_runs_reset(repetitions, time, agents):
    ...
    while i < repetitions:
        try: 
            wealth_coordinates = np.random.dirichlet(np.ones(3),size=1)[0].tolist()
            print(wealth_coordinates)
            df,pop = main('scholl', solver, wealth_coordinates, agents, time, 0, 0, ReinvestmentRate=0, tqdm_display=True, reset_wealth = True)
...
def det_pop_nolearning(repetitions, time, agents):
    ...
    for _ in range(repetitions):
        ...
        df,pop = main('scholl', solver, wealth_coordinates, agents, time, 0, 0, ReinvestmentRate=0, tqdm_display=True, reset_wealth = False)
        np.random.seed(seed)
        df2,pop2 = main('scholl', solver, wealth_coordinates, agents, time, 0, 0, ReinvestmentRate=0, tqdm_display=True, reset_wealth = False)
...
def det_pop_learning(repetitions, time, agents):
    ...
    for _ in range(repetitions):
        ...
        wealth_coordinates = np.random.dirichlet(np.ones(3),size=1)[0].tolist()
        np.random.seed(seed)
        df,pop = main('scholl', solver, wealth_coordinates, agents, time, PROBA_SELECTION, MUTATION_RATE, ReinvestmentRate=0, tqdm_display=True, reset_wealth = False)
        np.random.seed(seed)
        df2,pop2 = main('scholl', solver, wealth_coordinates, agents, time, PROBA_SELECTION, MUTATION_RATE, ReinvestmentRate=0, tqdm_display=True, reset_wealth = False)

...
def learning_runs_reset(repetitions, time, agents):
    ...
    while i < repetitions:
        ...
        print(wealth_coordinates)
        try: 
            df,pop,av = main(None, 'scholl', wealth_coordinates, agents, time, tqdm_display=True, reset_wealth = True)
...
def nolearning_runs_ext(repetitions, time, agents):
    ...
    while i < repetitions:
        try: 
            ...
            wealth_coordinates = np.random.dirichlet(np.ones(3),size=1)[0].tolist()
            print(wealth_coordinates)
            df,pop,av = main(None, 'extended', wealth_coordinates, agents, time, tqdm_display=True, reset_wealth = False)

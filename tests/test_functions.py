...
def learning_runs_reset(repetitions, time, agents):
    ...
    while i < repetitions:
        ...
        print(wealth_coordinates)
        try: 
            df = main("between", time, PROBA_SELECTION, agents, MUTATION_RATE, wealth_coordinates, tqdm_display=True, reset_wealth = True)
...
def nolearning_runs_reset(repetitions, time, agents):
    ...
    while i < repetitions:
        try: 
            wealth_coordinates = np.random.dirichlet(np.ones(3),size=1)[0].tolist()
            print(wealth_coordinates)
            df = main("static", time, 0, agents, 0, wealth_coordinates, tqdm_display=True, reset_wealth = True)
